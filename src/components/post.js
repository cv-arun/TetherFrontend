    ...
    return (
        ...
        <div key={postData._id} className='max-w-[690px] shadow my-1 bg-white rounded-lg flex flex-col'>
            <div className='flex mt-2 px-3'>
                ...
                <img className='h-[80px] w-[80px] min-w-[80px] min-h-80 object-cover rounded-full cursor-pointer' src={postData.user.picture} onClick={() => navigate(`/profiles/${postData.user._id}`)} loading="lazy" alt='failed to load profile' />
            ...
            {postData.images[0] && <div className='m-5 mb-0 overflow-y-auto flex scrollbar-hide snap-mandatory snap-x '>
                {postData.images.map((img, i) => (<div key={i} className=' min-w-full snap-center'><img className='max-h-[500px] object-contain w-full' src={img.url} alt='failed to load' loading="lazy" /></div>))}
            </div>}
